...
describe('Game', () => {
  ...
  describe('updateAvailableSquares', () => {
    test('updates gameShift status to in-progress', () => {
      ...
      game.setState = (updater) => {
        game.state = typeof updater === 'function' ? updater(game.state) : { ...game.state, ...updater }
      }

      const squares = [{ id: '0,10', row: 0, column: 10 }]
  ...
  describe('updatePlayerPosition', () => {
    test('resets gameShift to waiting after a move', () => {
      ...
      game.state = {
        ...
        gameShift: { status: 'in-progress', availableSquares: [{ id: '1,0' }], playerId: 1 },
      }
      game.setState = (updater) => {
        game.state = typeof updater === 'function' ? updater(game.state) : { ...game.state, ...updater }
      }
  ...
  describe('gameShift', () => {
    test('returns the active player and available squares', () => {
      const { container } = render(<Game />)
      // Verify the initial state is rendered as "waiting" (play button has pulsate class)
