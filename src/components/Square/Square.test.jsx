...
const player = { id: 1, name: 'John', color: 'blue', position: { row: 5, column: 3, place: null } }
...
const makeProps = (overrides = {}) => ({
  ...
  available: false,
  direction: '',
  gameShift: { player, availableSquares: [], status: 'waiting' },
...
describe('Square', () => {
  ...
  describe('click handling', () => {
    ...
    test('calls updatePlayerPosition after the animation when available', () => {
      ...
      MovementAnimation.mockImplementation(function() { this.move = () => newPosition })

      const path = ['5,3', '5,4']
      const gameShift = { player, availableSquares: [], status: 'in-progress' }
    ...
    test('animation timeout scales with path length', () => {
      ...
      // path has 3 steps → timeout = 3 * 500 = 1500ms
