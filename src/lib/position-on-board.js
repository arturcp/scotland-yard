class PositionOnBoard {
  ...
  insideBoard = (row, column) => row >= 0 && row < this.boardData.length && column >= 0 && column <= 22
  ...
  availableSquare = (row = this.row, column = this.column) => this.insideBoard(row, column) && parseInt(this.boardData[row][column]) > 0
}
