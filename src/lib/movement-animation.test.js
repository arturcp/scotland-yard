...
describe('MovementAnimation', () => {
  ...
  describe('move', () => {
    ...
    test('updates the player pin top and left via DOM after each step', () => {
      ...
      const pin = document.querySelector('#player-1')
      // top = row * squareHeight + paddingTop = 1 * 49 + 7 = 56
      expect(pin.style.top).toBe('56px')
      // left = col * squareWidth + paddingLeft + offset(id=1: 0) = 2 * 49 + 3 + 0 = 101
    ...
    test('animates each step with a 500ms delay', () => {
      ...
      expect(pin.style.top).toBe('56px') // first step

      vi.advanceTimersByTime(500)
      expect(pin.style.top).toBe('105px') // second step: 2 * 49 + 7 = 105
    })
  })
})
